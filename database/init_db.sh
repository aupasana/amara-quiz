...
cat database/amara_pada_english.utf8 | cut -f 1,6 -d, > database/tmp_english.csv
sqlite3 -separator ',' docker/amara.db ".import database/tmp_english.csv staging_translation"
sqlite3 docker/amara.db "update pada
set artha_english = ( select artha_english from staging_translation where pada.pada_uid = staging_translation.pada_uid )
